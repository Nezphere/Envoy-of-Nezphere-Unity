...
//					fastZip.ExtractZip(file, resultPath, null);
		...
		foreach (var dir in dirs) {
			...
			foreach (var file in files) {
				if (file.Substring(file.Length - 4, 4).ToLower() == ".osu") {
					...
					Debug.Log(string.Format("'{0}' : '{1}'", osuFile.BeatmapID, beatmapId));
					if (string.Equals(osuFile.BeatmapID, beatmapId))
						break;
					else
						osuFile = null;
				}
		...
		GetComponent<MusicPlayer>().Play(Path.Combine(osuFile.path, osuFile.AudioFilename));
