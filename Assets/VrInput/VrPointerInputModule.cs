...
namespace VrInput {
	public class VrPointerInputModule : BaseInputModule {
		public static VrPointerInputModule Instance { get; private set; }
		...
		private readonly Dictionary<VrPointer, ControllerData> controllerData =
			new Dictionary<VrPointer, ControllerData>();
		...
		public override void Process() {
			...
			foreach (var pair in controllerData) {
				...
				if (!UpdateCameraPosition(controller, data.PointerEvent))
					continue;

				if (data.PointerEvent == null)
					data.PointerEvent = new VrPointerEventData(eventSystem);
				else
...
//				data.PointerEvent.scrollDelta = new Vector2(0, Input.GetAxis(VrPlayer.WmrInput.WMR_A_R_PAD_Y) * 20);
